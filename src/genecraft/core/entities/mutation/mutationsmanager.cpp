...
QVariant MutationsManager::mutateEntity(const QVariant &entityVariant) {
    ...
    if(originsMap["family"].toString().compare("realspiderfamily", Qt::CaseInsensitive)==0)
    {
        QVariant newParams = this->mutateParams(paramsMap);
        entityMap.remove("params");
        entityMap.insert("params", newParams);
    }
